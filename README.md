...
Доступные эндпоинты API для сервиса магазина, включая базовые функции аутентификации, управления корзиной, оформления заказов и просмотра информации о товарах.
...
*   **Отправка email подтверждения заказа:** Отправляется при успешном создании заказа (`POST /api/orders/`) и при ручном изменении статуса на "confirmed" администратором.
*   **Обработка ошибок:** Корректная обработка ошибок аутентификации, авторизации, невалидных данных, отсутствующих объектов.
*   **Права доступа:** Анонимные пользователи не имеют доступа к корзине и заказам. Обычные пользователи видят только свои заказы. Администраторы могут подтверждать заказы.
